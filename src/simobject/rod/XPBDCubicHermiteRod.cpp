...
void XPBDCubicHermiteRod::setup()
{
    ...
    // MatXr hermite_inertia_global = MatXr::Zero(2*_num_elements+2, 2*_num_elements+2);
    // for (int i = 0; i < _num_elements; i++)
    // {
    //     hermite_inertia_global.block<4,4>(2*i, 2*i) += M_e;
    // }
    // _hermite_inertia_global_inv = hermite_inertia_global.inverse();
    ...
    // just keep 12x12 tridiagonal approximation of inverse
    // for (int i = 0; i < _num_nodes; i++)
    // {
    ...
    //         if (std::abs(i-j) > 1)
    //         {
    //             _inertia_mat_global_inv.template block<12,12>(12*i,12*j) = Eigen::Matrix<Real,12,12>::Zero();
    //         }
    //     }
    // }
    // std::cout << "Inertia mat (banded) inverse:\n" << _inertia_mat_global_inv << std::endl;

    // std::cout << "Hermite global:\n" << hermite_inertia_global << std::endl;
    // std::cout << "Hermite inverse:\n" << _hermite_inertia_global_inv << std::endl;
    // std::cout << "Scaled hermite inverse:\n" << 1/(_density * _area * _element_rest_length) * _hermite_inertia_global_inv << std::endl;
    ...
    int num_diag_blocks = elastic_constraints.size() + (int)_base_fixed + (int)_tip_fixed;
    ...
    // _mass_weighted_gradient_buffer.resize(elastic_constraints.size());

    // number of diagonals = bandwidth + 1
    _diagonals.resize(bandwidth+1);
    for (int i = 0; i < bandwidth+1; i++)
        _diagonals[i].resize(num_diag_blocks, Mat6r::Zero());

    _solver.setBandwidth(bandwidth);
    _solver.setNumDiagBlocks(num_diag_blocks);
}
...
void XPBDCubicHermiteRod::internalConstraintSolve(Real dt)
{
    ...
    // // iterate through elements and compute all the constraint gradients and the mass-weighted constraint gradients
    // // this will be used later in the delC
    // // i.e. each entry in _gradient_buffer will store a 
    // Real inv_element_mass = 1/(_density * _area * _element_rest_length);
    // Vec3r inv_element_rot_inertia = 1.0/(_density * _element_rest_length * Vec3r(_Ix, _Ix, _Iz).array());
    // for (int i = 0; i < _num_elements; i++)
    // {
    //     // inverse inertia for the element DOFs
    //     Mat4r inv_M_e = _hermite_inertia_global_inv.block<4,4>(2*i,2*i);

    //     for (int j = 0; j < NUM_GP; j++)
    //     {
    //         // constraint index that we are on (in terms of elastic constraints)
    //         int constraint_ind = NUM_GP*i + j;
    //         // compute elastic constraint gradient at this Gauss point
    //         const ElasticConstraintType::GradientMatType grad = elastic_constraints[constraint_ind].gradient();
    //         // the constraint gradient is organized into 6x3 blocks as follows:
    //         // | dC/p1 | dC/R1 | dC/p2 | dC/R2 | dC/p'1 | dC/R'1 | dC/p'2 | dC/R'2 |
    //         // i.e. normal DOF and then derivative DOF
    //         // we want to re-organize into the following seet of 6x3 blocks
    //         // | dC/p1 | dC/R1 | dC/p'1 | dC/R'1 | dC/p2 | dC/R2 | dC/p'2 | dC/R'2 |
    //         _gradient_buffer[constraint_ind].block<6,6>(0,0) = grad.block<6,6>(0,0);
    //         _gradient_buffer[constraint_ind].block<6,6>(0,6) = grad.block<6,6>(0,12);
    //         _gradient_buffer[constraint_ind].block<6,6>(0,12) = grad.block<6,6>(0,6);
    //         _gradient_buffer[constraint_ind].block<6,6>(0,18) = grad.block<6,6>(0,18);

            
            
    //         // there is no coupling between p, p' DOF and the R, R' DOF
    //         // furthermore, the p1-p1 coupling is the same as the R1-R1 coupling, p1-p2' coupling the same as R1-R2' coupling, etc.
    //         // for translational mass, multiply by rho * A * h
    //         // for rotational mass, multiply by rho * h * J
    //         for (int k1 = 0; k1 < 4; k1++)
    //         {
    //             Eigen::Matrix<Real, 6, 3> mC_block_trans = Eigen::Matrix<Real, 6, 3>::Zero();
    //             Eigen::Matrix<Real, 6, 3> mC_block_rot = Eigen::Matrix<Real, 6, 3>::Zero();
    //             mC_block_trans += inv_M_e(k1, 0) * inv_element_mass * grad.block<6,3>(0,0) +    // coupling with p1
    //                               inv_M_e(k1, 2) * inv_element_mass * grad.block<6,3>(0,6) +    // coupling with p2
    //                               inv_M_e(k1, 1) * inv_element_mass * grad.block<6,3>(0,12) +   // coupling with p'1
    //                               inv_M_e(k1, 3) * inv_element_mass * grad.block<6,3>(0,18);    // coupling with p'2

    //             mC_block_rot += inv_M_e(k1, 0) * grad.block<6,3>(0,3) * inv_element_rot_inertia.asDiagonal() +  // coupling with R1
    //                             inv_M_e(k1, 2) * grad.block<6,3>(0,9) * inv_element_rot_inertia.asDiagonal() +  // coupling with R2
    //                             inv_M_e(k1, 1) * grad.block<6,3>(0,15) * inv_element_rot_inertia.asDiagonal() + // coupling with R'1
    //                             inv_M_e(k1, 3) * grad.block<6,3>(0,21) * inv_element_rot_inertia.asDiagonal();  // coupling with R'2

    //             // put these blocks into their respective places
    //             _mass_weighted_gradient_buffer[constraint_ind].block<6,3>(0,6*k1) = mC_block_trans;
    //             _mass_weighted_gradient_buffer[constraint_ind].block<6,3>(0,6*k1+3) = mC_block_rot;
    //         }
    ...
    // Eigen::Matrix<Real,6,24> mass_weighted_fixed_base_grad = Eigen::Matrix<Real,6,24>::Zero();
    // if (_base_fixed)
    // {
    ...
    //     Eigen::Matrix<Real,6,24> full_grad = Eigen::Matrix<Real,6,24>::Zero();
    //     full_grad.block<6,6>(0,0) = _internal_constraints.get<Constraint::OneSidedFixedJointConstraint>().front().gradient();

    //     // compute the M^-1 * delC product
    //     mass_weighted_fixed_base_grad.block<6,3>(0,0) = _hermite_inertia_global_inv(0,0) * inv_element_mass * full_grad.block<6,3>(0,0);
    //     mass_weighted_fixed_base_grad.block<6,3>(0,3) = _hermite_inertia_global_inv(0,0) * full_grad.block<6,3>(0,3) * inv_element_rot_inertia.asDiagonal();
    //     mass_weighted_fixed_base_grad.block<6,3>(0,6) = _hermite_inertia_global_inv(1,0) * inv_element_mass * full_grad.block<6,3>(0,0);
    //     mass_weighted_fixed_base_grad.block<6,3>(0,9) = _hermite_inertia_global_inv(1,0) * full_grad.block<6,3>(0,3) * inv_element_rot_inertia.asDiagonal();
    //     mass_weighted_fixed_base_grad.block<6,3>(0,12) = _hermite_inertia_global_inv(2,0) * inv_element_mass * full_grad.block<6,3>(0,0);
    //     mass_weighted_fixed_base_grad.block<6,3>(0,15) = _hermite_inertia_global_inv(2,0) * full_grad.block<6,3>(0,3) * inv_element_rot_inertia.asDiagonal();
    //     mass_weighted_fixed_base_grad.block<6,3>(0,18) = _hermite_inertia_global_inv(3,0) * inv_element_mass * full_grad.block<6,3>(0,0);
    //     mass_weighted_fixed_base_grad.block<6,3>(0,21) = _hermite_inertia_global_inv(3,0) * full_grad.block<6,3>(0,3) * inv_element_rot_inertia.asDiagonal();
        
    //     // main diagonal is just delC * M^-1 * delC^T + alpha_tilde
    //     _diagonals[0][diag_block_ind] = mass_weighted_fixed_base_grad * full_grad.transpose();
    ...
    //         _diagonals[j+1][diag_block_ind] = _gradient_buffer[j] * mass_weighted_fixed_base_grad.transpose();
    //     }
    ...
    //                 _gradient_buffer[other] * _mass_weighted_gradient_buffer[this_ind].transpose();

    //             std::cout << "Diag " << diag_index << " block " << diag_block_ind << ": " << std::endl;
    //             std::cout << _gradient_buffer[other] << "\n multiplied by\n" << _mass_weighted_gradient_buffer[this_ind].transpose() << std::endl;
    //             std::cout <<"\nresult:\n" << _gradient_buffer[other] * _mass_weighted_gradient_buffer[this_ind].transpose() << std::endl;
    //             std::cout <<"\nplus alpha_tilde:\n" << _diagonals[diag_index][diag_block_ind].diagonal() + alpha_tilde << std::endl;
    //         }
    ...
    //             std::cout << "ADJACENT CONSTRAINT DIAG INDEX: " << diag_index << std::endl;

    //             // the index of the node shared by the constraints
    //             int shared_node_ind = i+1;

    //             // extract the block associated with the "last" node affected by the constraint
    //             Eigen::Matrix<Real,6,12> this_block = _mass_weighted_gradient_buffer[this_ind].template block<6,12>(0,12);
    //             // extract the block associated with the "first" node affected by the constraint
    //             Eigen::Matrix<Real,6,12> other_block = _gradient_buffer[other].template block<6,12>(0,0);

    //             _diagonals[diag_index][diag_block_ind] = other_block * this_block.transpose();
    //         }
    ...
    //     _dx.template block<24,1>(0,0) += mass_weighted_fixed_base_grad.transpose() * constraint_dlam;
    ...
    //         Vec6r constraint_dlam = _dlam.template block<6,1>(6*constraint_ind, 0);
    //         _dx.template block<24,1>(12*first_node_ind,0) += _mass_weighted_gradient_buffer[elastic_constraint_ind].transpose() * constraint_dlam;
    ...
    // std::cout << "True delC * M^-1:\n" << _delC_mat * _inertia_mat_global_inv << std::endl;
    // std::cout << "delC * M^-1 gradient buffer: " << std::endl;
    // for (int i = 0; i < elastic_constraints.size(); i++)
    // {
    //     std::cout << _mass_weighted_gradient_buffer[i] << std::endl;
    // }

    MatXr LHS_mat = _delC_mat * _inertia_mat_global_inv * _delC_mat.transpose();

    LHS_mat.diagonal() += _alpha/(dt*dt);
    // std::cout << "LHS mat:\n" << LHS_mat << std::endl;

    // for (int i = 0; i < _diagonals.size(); i++)
    // {
    //     std::cout << "\nDiagonal " << i << ": " << std::endl;
    //     for (int j = 0; j < _diagonals[i].size(); j++)
    //     {
    //         std::cout << _diagonals[i][j] << std::endl;
    //     }
    // }
