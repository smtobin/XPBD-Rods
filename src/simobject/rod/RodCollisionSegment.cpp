...
AABB RodCollisionSegment::boundingBox() const
{
    ...
    AABB bbox;
    bbox.min[0] = std::min(p1_pos[0], p2_pos[0]) - 10*radius();
    bbox.min[1] = std::min(p1_pos[1], p2_pos[1]) - 10*radius();
    bbox.min[2] = std::min(p1_pos[2], p2_pos[2]) - 10*radius();

    bbox.max[0] = std::max(p1_pos[0], p2_pos[0]) + 10*radius();
    bbox.max[1] = std::max(p1_pos[1], p2_pos[1]) + 10*radius();
    bbox.max[2] = std::max(p1_pos[2], p2_pos[2]) + 10*radius();
