...
Real dcubicHermiteN2(Real s_hat) { return 1-s_hat*s_hat; }
...
CubicHermiteRodElement::StrainGradientMatType CubicHermiteRodElement::strainGradient(Real s_hat) const
{
    ...
    dthetaprime_dRi[1] = dshat_ds() * _bases_derivatives[2](s_hat) * gam_inv.transpose();
    ...
    for (int i = 0; i < 4; i++)
    {
        Mat3r dv_dp = R.transpose() * dshat_ds() * _bases_derivatives[i](s_hat);
        grad.template block<3,3>(0,6*i) = dv_dp;
    }
    

    // w.r.t orientation
    grad.template block<3,3>(0,3) = RT_dp_ds_R * (exp_theta.transpose() + gam_theta * dtheta_dRi[0]);
    grad.template block<3,3>(0,9) = RT_dp_ds_R * gam_theta * dtheta_dRprimei[0];
    grad.template block<3,3>(0,15) = RT_dp_ds_R * gam_theta * dtheta_dRi[1];
    ...
    grad.template block<3,3>(3,9) = dexpmap_contract_j * dtheta_dRprimei[0] + gam_theta * dthetaprime_dRprimei[0];
    grad.template block<3,3>(3,15) = dexpmap_contract_j * dtheta_dRi[1] + gam_theta * dthetaprime_dRi[1];
