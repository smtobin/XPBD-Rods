...
RodRigidBodyCollisionConstraint::RodRigidBodyCollisionConstraint(
    ...
    : XPBDConstraint<1, 3, 0>({p1, p2, com_rb}, 1.0e-10*AlphaVecType::Ones()), 
...
RodRigidBodyCollisionConstraint::ConstraintVecType RodRigidBodyCollisionConstraint::evaluate() const
{
    ...
    const Vec3r R_diff = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation);
    const Mat3r R_local = Math::Plus_SO3(_oriented_particles[0]->orientation, _beta*R_diff);
    const Vec3r cp_rod = (1-_beta)*_oriented_particles[0]->position + _beta*_oriented_particles[1]->position + R_local*_cp_local_rod;
...
RodRigidBodyCollisionConstraint::GradientMatType RodRigidBodyCollisionConstraint::gradient() const
{
    ...
    const Vec3r R_diff1 = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation);
    ...
    const Mat3r R_local_skew_cp1 = _oriented_particles[0]->orientation * exp_Rdiff1 * Math::Skew3(_cp_local_rod);
...
void RodRigidBodyCollisionConstraint::applyFriction(Real lambda_n) const
{
    ...
    const Vec3r R_diff1 = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation);
    const Mat3r exp_Rdiff1 = Math::Exp_so3(_beta*R_diff1);
    const Mat3r R_local1 = _oriented_particles[0]->orientation * exp_Rdiff1;
    const Vec3r cp_rod = (1-_beta)*_oriented_particles[0]->position + _beta*_oriented_particles[1]->position + R_local1*_cp_local_rod;

    // previous location of contact point on rod
    const Vec3r R_diff_prev1 = Math::Minus_SO3(_oriented_particles[1]->prev_orientation, _oriented_particles[0]->prev_orientation);
    const Mat3r R_local_prev1 = Math::Plus_SO3(_oriented_particles[0]->prev_orientation, _beta*R_diff_prev1);
    const Vec3r prev_cp_rod = (1-_beta)*_oriented_particles[0]->prev_position + _beta*_oriented_particles[1]->prev_position + R_local_prev1*_cp_local_rod;
    ...
    Vec3r v_cp_rod = (1-_beta)*_oriented_particles[0]->lin_velocity + _beta*_oriented_particles[1]->lin_velocity + dRcp_dR1*_oriented_particles[0]->ang_velocity + dRcp_dR2*_oriented_particles[1]->ang_velocity;
...
OneSidedRodRigidBodyCollisionConstraint::OneSidedRodRigidBodyCollisionConstraint(
    ...
    : XPBDConstraint<1, 2, 0>({p1, p2}, 1.0e-10*AlphaVecType::Ones()),
...
OneSidedRodRigidBodyCollisionConstraint::GradientMatType OneSidedRodRigidBodyCollisionConstraint::gradient() const
{
    // gradients w.r.t. positions of rod
    Vec3r dC_dp1 = -(1-_beta)*_n;
    Vec3r dC_dp2 = -_beta*_n;
