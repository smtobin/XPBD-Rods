...
//     const Vec3r dp = _oriented_particles[1]->position - _oriented_particles[0]->position;
//     // std::cout << "dp:\n" << dp << std::endl;
//     ConstraintVecType C;
//     C(Eigen::seqN(0,3)) = (_oriented_particles[0]->orientation.transpose() + _oriented_particles[1]->orientation.transpose()) * 0.5 * dp / _dl - Vec3r(0,0,1);
//     C(Eigen::seqN(3,3)) = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation) / _dl;
...
//     const Mat3r dCv_dp1 = -0.5*(_oriented_particles[0]->orientation.transpose() + _oriented_particles[1]->orientation.transpose()) / _dl;
//     const Mat3r dCv_dp2 = -dCv_dp1;
    
//     const Vec3r dp = _oriented_particles[1]->position - _oriented_particles[0]->position;
//     const Mat3r dCv_dor1 = 0.5*Math::Skew3(_oriented_particles[0]->orientation.transpose() * dp / _dl);
//     const Mat3r dCv_dor2 = 0.5*Math::Skew3(_oriented_particles[1]->orientation.transpose() * dp / _dl);

//     const Vec3r dtheta = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation);
...
//     if (use_cache)
//     {
//         if (node_ptr == _oriented_particles[0])
//         {
//             return _cached_gradients[0];
//         }
//         else if (node_ptr == _oriented_particles[1])
//         {
...
//     else if (node_ptr == _oriented_particles[1])
//     {
//         const Mat3r dCv_dp2 = 0.5*(_oriented_particles[0]->orientation.transpose() + _oriented_particles[1]->orientation.transpose()) / _dl;
//         const Vec3r dp = _oriented_particles[1]->position - _oriented_particles[0]->position;
//         const Mat3r dCv_dor2 = 0.5*Math::Skew3(_oriented_particles[1]->orientation.transpose() * dp / _dl);
//         const Vec3r dtheta = Math::Minus_SO3(_oriented_particles[1]->orientation, _oriented_particles[0]->orientation);
//         const Mat3r jac_inv = Math::ExpMap_InvRightJacobian(dtheta);
